...
- **Volta constraints**: pinned CUDA-12/Volta-compatible base image; **`--enforce-eager` is
  ...
  learned the hard way). The scheduler always includes it in `vllm_args` for a cluster instance.
...
per-node `ensure_instance` commands. The agents do locally what `run.sh` + the manager did
...
admin scheduler:
  ...
  • pick head node, allocate port, assemble vllm_args (+ --enforce-eager)
        │
        ├── ensure_instance{role: head,   ray:{head_addr,port}, layout, vllm_args...} → node A agent
        └── ensure_instance{role: worker, ray:{head_addr,port}, ...}                  → node B agent
                                   │
...
> Stopping an instance without bouncing Ray can **leak the placement group**. A later start then
...
- `stop_instance` carries an explicit `ray: bool`. The admin's default teardown for a
  cluster instance is `ray: true` (bounce the Ray runtime with the instance) to avoid the leak;
  a lighter `ray: false` stop is available when reusing the runtime is known-safe.
- On node loss, the reconciler tears the instance down **and** clears Ray state on the survivors
  before any reschedule, so job *N+1* starts clean.
- The agent reaps orphaned vLLM/driver processes and frees GPU reservations on `stop_instance`
