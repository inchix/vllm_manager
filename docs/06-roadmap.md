...
## Phase 2 — Scheduler + `ensure_instance` (single-node first) — 🟡 logic done, HW gate pending

- Admin scheduler computes layout (TP/PP/partition) and emits `ensure_instance`.
...
- **Gate (needs container + GPUs):** launch/stop a single-node instance on COVID purely via CCP;
  admin restart → instance re-adopted by the reconciler; orphaned-process/GPU-reservation leaks
  gone.
...
- ✅ `serve_storage` (starts `modelfsd`) + `mount_storage`/`unmount_storage` in the runner;
  ...
  canonical path before an instance loads, gating the launch on it.
...
- **Gate:** ebola loads a model entirely through the coordinated mount; storage-node-down is
  detected and blocks new instances with a clear message; a client read against a killed server
  errors promptly (no D-state) — verified by killing the server mid-read.

## Phase 4 — Multi-node instance + auto-recovery (the payoff)

- Fan-out `ensure_instance` across COVID+ebola: TP=2 × PP=2, `--enforce-eager`, memory-weighted
...
- Reconciler handles node loss: mark FAILED, clear Ray PG on survivors, auto-rejoin on
  reconnect.
- **Gate:** serve Devstral at 64k across both boxes via CCP; reboot ebola under load → instance
  fails, ebola auto-rejoins on boot and the reconciler restores the instance **with no human
...
- **Gate:** a fresh two-box bring-up using only `setup.sh` + the UI, no manual `run.sh`; and
  ...
  next instance start without touching any file.
...
- Auto-rescheduling instances onto survivors (start with hold+alert; revisit later).
